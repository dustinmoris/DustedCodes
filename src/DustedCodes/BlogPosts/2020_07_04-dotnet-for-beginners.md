...
Why is .NET so complicated? Well, the answer is not that easy itself :). It's mostly history, legacy baggage, some bad decisions along the way and what I believe an overly eager desire (almost urgency) to push boundaries for boundaries' sake. I will try to name a few selected issues from my personal observation, describe the problems which I have perceived and try to provide some constructive suggestions which I think could be good improvements for the future.
 
### The Languages

If a person sets out to learn C# (like the author from the question above), what do they learn? Is it C# or .NET? The answer is both. C# doesn't exist without .NET and you cannot program .NET without C# (or F# or VB.NET). This is not a problem in itself, but certainly where some of the issues begin. A new beginner doesn't just learn C# but also has to learn the inner workings of .NET. Things get even more confusing when C# isn't the chosen language to start with. The loose relationships between .NET languages don't really matter because they compile into IL and become cross compatible. [Except when they aren't](https://github.com/dotnet/vblang/issues/300).

The [BCL (Base Class Libraries)](https://docs.microsoft.com/en-us/dotnet/standard/framework-libraries#base-class-libraries) provide the foundation for all three languages, yet they are entirely written in only C#. That's not really an issue, unless entire features were only written with one language in mind (C#) and are extremely cumbersome to use from another. F# still doesn't have [native support for `Task` and `Task<T>`](https://github.com/fsharp/fslang-suggestions/issues/581), converting between `System.Collection.*` classes and F# types is a painful undertaking and F# functions and .NET `Action<T>` objects don't map very well.
...
It makes a lot of sense and it is a great achievement, but the angle taken is causing more harm than help. While it makes a lot of sense to walk on a stage and boast to potential customers how your product can solve all their existing and future problems, it's not the best approach when you actually want to on-board thousands of new developers.

My suggestion would be to clearly separate the marketing and developer material. It's rarely the case that a new .NET developer comes along and wants to be confronted with a whole lot of options to choose from before they can get started. Most newcomers just want to get the bare bones minimum set up to get up and running. When you are just about to start learning C# then it absolutely doesn't matter if you are using the fanciest latest Visual Studio or the hottest client side framework (Blazor) or the latest iteration of App UI development (MAUI). Keep that noise away from beginners for as long as possible. Don't worry about your advanced users, because they will know how and where to find it without Microsoft constantly having to put it up in their face. After all it works for all other programming communities as well, so why wouldn't it work for .NET developers too?
