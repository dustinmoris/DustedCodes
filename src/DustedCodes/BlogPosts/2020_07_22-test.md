...
## Composing a CI/CD pipeline for .NET Core packages
...
On this premise I decided that each commit, regardless if it happened on a `feature/*`, `hotfix/*` or other branch, should trigger a run. Pull requests should trigger a CI run as well. It's a great way of validating the changes of a PR before merging. As a matter of fact, it's highly recommended to enforce this rule through GitHub itself.
...
![GitHub Branch Protection Rules](https://storage.googleapis.com/dusted-codes/images/blog-posts/2020-06-28/github-branch-protection-rules.png)
...
We can configure a GitHub Action to trigger builds for commits on all branches and pull requests by providing the `push` and `pull_request` trigger and leave the branch definition blank:
...
.NET Core is cross platform so it's no surprise that a NuGet library is expected work on Linux, macOS and Windows at a minimum.

This can be configured by setting up a build matrix like this:
...
![GitHub Action Build Artifacts](https://storage.googleapis.com/dusted-codes/images/blog-posts/2020-06-28/github-build-artifacts.png)
...
### Push pre-release packages to GitHub
...
In the context of a NuGet package a &quot;nightly release&quot; is a NuGet library which normally gets pushed to a public NuGet feed which is just like the official [NuGet Gallery](), but not the gallery itself. This is a common pattern for .NET Core libraries because developers can configure more than one NuGet feed in their project via a `NuGet.config` file (see the [nuget.config reference](https://docs.microsoft.com/en-us/nuget/reference/nuget-config-file) for more information) and therefore consume a nightly build package via the same commands and tooling as an official release. Most commonly I've seen self hosted [ProGet]() feeds or cloud hosted [MyGet]() feeds to distribute &quot;nightly builds&quot; alongside the official NuGet gallery. However, GitHub's own (and relatively new) [Packages Feed]() makes an attractive alternative.

A NuGet package can be pushed to GitHub's packages feed with the help of this workflow configuration:
...
I've created a complete separate job called `prerelease` in the workflow definition for this purpose alone. The name is completely arbitrary and can be swapped for anything else instead. Because I don't want the `prerelease` job to run in parallel to the `build` job and only run if the `build` job finished successfully I had to make this clear:

```
needs: build
```

Note that I had to specify here `build` because this was the chosen name for the initial job, however just like `prerelease` this is an arbitrary name and can be changed to your custom needs.
